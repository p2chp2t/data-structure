...
/*
    ...
    Input:
        (?쁦nsert??string): insert string into the current max heap

    Output:
        Letters in the heap in a node number order, in a string separated with
        the white space
        There will be no ?쐃rror case??such as overflow, underflow or empty heap
*/
...
/*
    ...
    Input:
        (?쁦nsert??string): insert integer into the current max heap
        (?쁡elMax??NULL): delete highest priority letter from current binary max heap
