...
/*
    ...
    Description:
        Implement a function that sorts a given array using the insertion sort
   ...
   print the array). You don?셳 need to consider exceptional cases such as
...
/*
    ...
    Description:
        Implement a function that shows process of recursive merge sort
   algorithms and counts the number of ?쁡rop??operations. It will sort a given
   array in ascending order. You can modify sort.cpp and sort.h files for this
   problem.
    ...
    Input:
        ('insert',integer): insert integer into the array
        ('mergeSort',NULL): sort the array using the merge sort algorithm, and
   count ?쁡rop??operations.

    Output:
        Every value in the array for each sorting step including the initial
   step, string separated with the white space (please use built-in function to
   print the array). The number of ?쁡rop??operations.
*/
...
/*
    ...
    Description:
        Implement functions that inserts and deletes an element into a
   duplication-aware binary search tree. You can modify dabst.cpp and dabst.h
   files for this problem.
    ...
    Output of DABSTree::remove
        Print -1 if the key does not exist in the tree, and do not delete any
   element

    task_3 prints
        The results of preorder and inorder traversal of the current constructed
   tree when each operation performed successfully. -1, when DABSTree::remove is
   called and the key does not exist in the tree.
*/
...
/*
    ...
    Description:
        ...
        The insert and remove might cause the AVL tree to violate its properties
   (imbalance). Your code should be able to resolve the various imbalance type
   (LL, LR, RL, RR) of the AVL tree. You can modify avl.cpp and avl.h files for
   this problem.

        TODO: Implementation AVLTree::insert function
        TODO: Implementation AVLTree::remove function

        Hint : suggest you to call 'insert' and 'remove' function in
   BinarySearchTree class
    ...
    task_4 prints
        ...
        the preorder and inorder traversal of the constructed tree after *each*
   insertion/deletion.
*/
...
/*
    ...
    Description:
      Let?셲 consider multi-way search tree (MST). When inserting
...
/*
    ...
    Description:
        ...
        If the key already exists in the B-tree, you need to delete an element
   having that key. Your B-tree should maintain the B-tree property even after
   the insertion or deletion. After finishing every insertion/deletion process,
   print the constructed B-tree through postorder traversal method. If there is
   no element in the B-tree, print -1. You can modify B_tree.cpp and B_tree.h
   files for this problem.

        TODO: Implementation BTree class which has has_elem, insert_elem,
   delete_elem
